...
os.makedirs("/work/FAC/FGSE/IDYST/tbeucler/downscaling/mdefez/DEM/zip", exist_ok=True) # Where to stock the downloaded zip files

# Put an access token here, be careful it usually resets every 10 minutes
access_token = ""

df_coord_id = pd.read_excel("/work/FAC/FGSE/IDYST/tbeucler/default/maxdefez/Precipitation_Dowscaling/DEM/coordinates_with_id.xlsx") # Read the df with the product id and corresponding coordinates
...
for k in range(len(df_coord_id)):
    ...
    if pd.isna(id) == False:
        ...
        if response.status_code == 200:
            with open(f"/work/FAC/FGSE/IDYST/tbeucler/downscaling/mdefez/DEM/zip/{coord}.zip", "wb") as file:
