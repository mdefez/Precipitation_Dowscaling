...
name_of_the_run = f"{name_loss[base_loss]}_input_{n_inputs}_n_days_{n_days_train}_attention_{strat_attention}_delay_constraint_{treshold_constraint}_lr_{learning_rate}_epochs_{epochs}"
...
if training:
    ...
    for k in range(len(splits)):
        ...
        tool.save_model(weights, name_of_the_run, spatial_factor=spatial_factor, temp_factor=temp_factor)     # Save model at each split (so that we eventually don't have to wait the end of the CV). To be deleted.
        if normalizing:
