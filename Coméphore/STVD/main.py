...
def main_function():
    ...
    from Coméphore.Deterministic.loss import LossTest, PercentileDifferenceLoss, Log_spectral_distance, EarthMovingDistance, SSIM, PITD
    import Coméphore.Deterministic.tools as tool 
    from Coméphore.STVD.test import test 
    from Coméphore.STVD.cross_validation import k_fold, simple_training
    ...
    name_of_the_run = f"diffusion_{use_diffusion}_input_{n_inputs}_n_days_{n_days_train}_{n_days_test}_attention_{strat_attention_deter+strat_attention_diffu}_window_{window_size}_heads_{nb_heads}_delay_constraint_{treshold_constraint_deter}_beta_{beta[0]}_{beta[1]}_function_{name_function}_lr_{learning_rate}_epochs_{epochs}_cross_val_{cross_val}"
    ...
    if training:
        ...
        if normalizing:
            tool.save_transfo(output_path = working_directory + "/Deterministic/normalization", 
